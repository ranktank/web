...
module.exports = Backbone.View.extend({
  ...
  initialize: function(options) {
    ...
    if (options.sortable) {
      return this.$el.sortable(options.sortable).on({
        sortremove: this.sortRemoveItem.bind(this),
        sortreceive: this.sortAddItemElement.bind(this)
      });
    }
  },
  events: {
  ...
  elementId: function(item) {
    return "" + this.idPrefix + item.id;
  },
  itemId: function(element) {
    return Number(element.attr('id').replace(this.idPrefix, ''));
  },
  addItem: function(item) {
    var existing, itemView;
    existing = this.$el.find("#" + (this.elementId(item)));
    if (!existing.length) {
      itemView = new ItemView({
        model: item,
        id: this.elementId(item),
        tagName: 'li',
  ...
  sortRemoveItem: function(event, ui) {
    return this.collection.remove({
      id: this.itemId(ui.item)
    });
  },
  sortAddItemElement: function(event, ui) {
    return ui.item.trigger('moveitem', {
      at: ui.item.index()
    });
  },
  addItemModel: function(event, at) {
    return this.collection.add(event.result, at);
  }
});
